	...
	if (input.files && input.files[0]) {
		...
		reader.onload = function (e) {
			...
			img.onload = function () {
				...
				$('#takenPicture').css("transform", "rotate(" + orientationFromExif + "deg)").css("background-position", "unset").css("margin-top", 0);
	...
	if (input.files && input.files[0]) {
		...
		reader.onload = function (e) {
			...
			img.onload = function () {
				...
				$('#takenPicture').css('background-position', '-' + afterScanFactor * cardData['position_x'] + 'px ' + '-' + afterScanFactor * cardData['position_y'] + 'px');
...
function countCardDimensions(result, scaleFactor) {
	var cardData = [];
	cardData['position_x'] = Math.min(result.creditCardCoordinate[0].p0.x, result.creditCardCoordinate[0].p1.x, result.creditCardCoordinate[0].p2.x, result.creditCardCoordinate[0].p3.x) / scaleFactor;
	cardData['position_y'] = Math.min(result.creditCardCoordinate[0].p0.y, result.creditCardCoordinate[0].p1.y, result.creditCardCoordinate[0].p2.y, result.creditCardCoordinate[0].p3.y) / scaleFactor;
	cardData['position_width'] = (Math.max(result.creditCardCoordinate[0].p0.x, result.creditCardCoordinate[0].p1.x, result.creditCardCoordinate[0].p2.x, result.creditCardCoordinate[0].p3.x) / scaleFactor - cardData['position_x']);
	cardData['position_height'] = (Math.max(result.creditCardCoordinate[0].p0.y, result.creditCardCoordinate[0].p1.y, result.creditCardCoordinate[0].p2.y, result.creditCardCoordinate[0].p3.y) / scaleFactor - cardData['position_y']);
...
function countRotation(result, cardData, scale) {
	...
	if (distanceB < distanceA && distanceB < distanceC && distanceB < distanceD) {
		...
		$("#takenPicture").css("margin-top", -(scale*cardData['position_height'] - scale*cardData['position_width']) / 2);
	}
...
function parseReturnedOcrData(result) {
	...
	var cardDataNameValue = result.result.split(/\r|\n/);
	var cardDataTemp = [];
	...
	var cardData = countCardDimensions(result, scaleFactor);
...
$(document).ready(function () {
	...
	if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
		...
		$('#takeAPicture').click(function() {
			...
			$('#takenPicture').css('background-image', 'url("' + dataUrl + '")').css("transform", "rotate(270deg)").css('width', 300 * $('#video').width() / $('#video').height() + 'px').css('height', '300px');
					...
					if (result.result == '') {
						...
						if (fillForm(cardData) == false) {
							...
							setTimeout(function () {
								var afterScanFactor = 180/Math.min(cardData['position_width'], cardData['position_height']);
								countRotation(result, cardData, afterScanFactor);
								$('#takenPicture').css('width', cardData['position_width'] + "px").css('height', cardData['position_height'] + "px").css('background-position', '-' + cardData['position_x'] + 'px ' + '-' + cardData['position_y'] + 'px');
							}, 1500);
						}
					}
				},
